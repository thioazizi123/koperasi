...
@section('header_title', 'Pencairan Dana')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-xl font-bold text-slate-800">Antrean Pencairan</h3>
            <p class="text-sm text-slate-500">Tugas bagian keuangan untuk menyiapkan dana bagi anggota yang telah disetujui.</p>
        </div>
        <span class="px-3 py-1 bg-kop-blue-50 text-kop-blue-600 rounded-full text-xs font-bold ring-1 ring-kop-blue-100">5 Siap Cair</span>
    </div>
        ...
        <table class="w-full text-left">
            <thead class="bg-slate-50">
                <tr class="text-slate-400 text-xs uppercase tracking-wider border-b border-gray-100">
                    ...
                    <th class="px-6 py-4 font-medium">Nilai Pencairan</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-slate-500">Transfer Bank</span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <form action="{{ route('finance.disburse.action', $request) }}" method="POST">
                            @csrf
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center text-slate-400 italic">Tidak ada antrean pencairan saat ini.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
